...
function PreSearch() {
  if (GameController.GameOver != BOOL.TRUE) {
    ...
    $('#ThinkingImageDiv').append('<image src="think3.png" id="ThinkingPng"/>')
    setTimeout(function () {
      StartSearch()
    }, 200)
  }
...
function RemoveGUIPiece(sq) {
  ...
  $('.Piece').each(function (index) {
    ...
    if (
      RanksBrd[sq] == 7 - Math.round($(this).position().top / 60) &&
...
function AddGUIPiece(sq, pce) {
  ...
  pieceFileName = '' + SideChar[PieceCol[pce]] + PceChar[pce].toUpperCase() + '.png'
  imageString =
    '<image src="' +
    pieceFileName +
...
function MoveGUIPiece(move) {
  ...
  $('.Piece').each(function (index) {
    //console.log( "Picture:" + index + ": " + $(this).position().top + "," + $(this).position().left );
    if (
      RanksBrd[flippedFrom] == 7 - Math.round($(this).position().top / 60) &&
...
function initBoardSquares() {
  ...
  for (rankIter = RANKS.RANK_8; rankIter >= RANKS.RANK_1; rankIter--) {
    ...
    for (fileIter = FILES.FILE_A; fileIter <= FILES.FILE_H; fileIter++) {
      ...
      divString =
        '<div class="Square clickElement ' + rankName + ' ' + fileName + ' ' + lightString + '"/>'
