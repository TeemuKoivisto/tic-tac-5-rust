...
function PrSq(sq) {
  ...
  var sqStr =
    String.fromCharCode('a'.charCodeAt() + file) + String.fromCharCode('1'.charCodeAt() + rank)
  return sqStr
}
...
function PrMove(move) {
  ...
  MvStr =
    String.fromCharCode('a'.charCodeAt() + ff) +
    String.fromCharCode('1'.charCodeAt() + rf) +
    String.fromCharCode('a'.charCodeAt() + ft) +
    String.fromCharCode('1'.charCodeAt() + rt)
  ...
  for (index = brd_moveListStart[brd_ply]; index < brd_moveListStart[brd_ply + 1]; ++index) {
    ...
    if (FROMSQ(Move) == from && TOSQ(Move) == to) {
      ...
      if (PromPce != PIECES.EMPTY) {
        if (
          (PromPce == PIECES.wQ && brd_side == COLOURS.WHITE) ||
          (PromPce == PIECES.bQ && brd_side == COLOURS.BLACK)
        ) {
          found = BOOL.TRUE
          break
        }
