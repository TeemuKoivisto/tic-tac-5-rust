...
function minimax(
  ...
  iterations += 1
  if (board.update_cell_owner(cell, player)) {
...
export const gameActions = {
  ...
  playerSelectCell(x: number, y: number) {
    ...
    let ended = true
    if (b.update_cell_owner(c, playerNumber)) {
  ...
  evaluateAiMove() {
    ...
    if (b.update_cell_owner(chosenCell, aiNumber)) {
